...
Reusable analytical procedures stored globally and activated per-message. Full vertical slice: DB migration v16 (`skills` table with COLLATE NOCASE, CHECK constraints, JSON1 validation) → Zod schemas with `.superRefine()` cross-field validation (structured ↔ outputSchema) → Repository with merged-state validation on update → Prompt injection (between shared protocols and domain protocols, with protocol precedence enforcement, 12K char budget) → IPC handlers (10 channels: CRUD + toggle + import/export + `list-enabled-for-domain`) → Preload bridge → Zustand store (`activeSkillIdByDomain` for domain-scoped selection, per-domain cache with 5-min TTL) → UI (SkillSelector chips, SkillEditor form, SkillLibraryDialog with search/filter/toggle/import/export). Import/export uses `.skill.md` format with frontmatter + fenced outputSchema block. Skills support freeform or structured JSON output with schema enforcement, and tool hints for recommending specific tools.

**Plugin-aware skill visibility**: Plugin skills respect three layers of enabled state: skill `is_enabled`, plugin global `is_enabled`, and per-domain `plugin_domain_assoc.is_enabled`. `listEnabledForDomain(domainId)` JOINs all three tables. `getEffectiveEnabled(skillId, domainId)` returns a reason enum (`OK`, `SKILL_DISABLED`, `PLUGIN_MISSING`, `PLUGIN_DISABLED_GLOBAL`, `PLUGIN_DISABLED_DOMAIN`) used as a chat:send safety net. `COALESCE(pda.is_enabled, 1)` defaults to visible when no per-domain row exists (plugin globally ON → visible everywhere unless explicitly disabled per-domain).

**Store-level cache invalidation**: The `skills:changed` IPC event is emitted after ALL skill and plugin mutations (create/update/delete/toggle/install/uninstall/enable-for-domain/disable-for-domain). A **module-level listener** on `useSkillStore` (outside the Zustand `create()` call) clears both `skillsByDomain` and `skillsLoadedAtByDomain` on every event — clearing both prevents stale skill arrays from rendering before async IPC refetch completes. This survives SkillSelector unmount/remount cycles — critical because users toggle plugins on the Plugins page while SkillSelector lives on the chat page. The SkillSelector also has its own component-level `onChanged` listener for immediate force-refetch when it IS mounted.
...
| `src/skills/repository.ts` | `SkillRepository` — CRUD with merged-state validation on update, COLLATE NOCASE duplicate guard, JSON roundtrip for toolHints, toggleEnabled, `listEnabledForDomain()` (3-way JOIN: skills+plugins+plugin_domain_assoc), `getEffectiveEnabled()` (returns reason enum), `EffectiveSkillReason` type |
...
| File | Purpose |
|------|---------|
| `src/main/ipc-handlers.ts` | 95+ IPC handlers: domains, KB, chat, briefing, intake, protocols, sessions, relationships, gap flags, decisions, audit, advisory, skills (incl. `list-enabled-for-domain`, `emitSkillsChanged()` after all skill+plugin mutations), missions, plugins (15 handlers: CRUD, install, domain assoc, marketplace), Gmail (incl. `enrichWithAttachments` for email attachment text extraction), GTasks, settings, file text extraction |
...
| `src/renderer/pages/DomainListPage.tsx` | Landing page when no domain selected — health chip strip, Skills/Plugins/Missions toolkit overview cards (3-card responsive grid with navigation actions), brainstorming tip, briefing link |
...
| `src/renderer/stores/skill-store.ts` | Zustand store for skills: per-domain cache (`skillsByDomain`, `skillsLoadedAtByDomain`), `fetchSkills(domainId, force?)` with 5-min TTL, `activeSkillIdByDomain` (domain-scoped selection), auto-clear stale selections. Module-level `skills:changed` listener invalidates all caches (survives component unmount) |
| `src/renderer/components/SkillSelector.tsx` | Chip-based skill selector above chat textarea; per-message activation with auto-clear after send. Component-level `skills:changed` listener for immediate refetch when mounted |
| `src/renderer/components/SkillLibraryDialog.tsx` | Full CRUD dialog: search, filter (all/enabled/disabled/custom/plugin), toggle, edit, delete with confirm, import/export. Separates custom skills from plugin skills in the list |
...
| `skill:list-enabled` | renderer→main | List enabled skills globally (legacy, checks plugin state) |
| `skill:list-enabled-for-domain` | renderer→main | List enabled skills for domain (JOINs plugins + plugin_domain_assoc) |
...
| `skills:changed` | main→renderer | Emitted after any skill or plugin mutation (create/update/delete/toggle/install/uninstall/enable/disable-for-domain) |
