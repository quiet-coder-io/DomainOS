...
**Store-level cache invalidation**: The `skills:changed` IPC event is emitted after ALL skill and plugin mutations (create/update/delete/toggle/install/uninstall/enable-for-domain/disable-for-domain). A **module-level listener** on `useSkillStore` (outside the Zustand `create()` call) clears `skillsLoadedAtByDomain` on every event. This survives SkillSelector unmount/remount cycles — critical because users toggle plugins on the Plugins page while SkillSelector lives on the chat page. The SkillSelector also has its own component-level `onChanged` listener for immediate force-refetch when it IS mounted.
